...
import {
  ...
  faMap, 
  faBars, faCircleQuestion, faCircleExclamation,
  faGlobe,
            ...
            <header className={isMenuOpen ? 'scrolled' : ''}>
                ...
                <section className="navy">
                    <nav className="navbar">
                        <Link to="/map" className="nav-item" style={{ '--i': 0 }}>{t('header.map')}</Link>
                        <Link to="/craigslist" className="nav-item" style={{ '--i': 1 }}>{t('header.craigslist')}</Link>
                        <LanguageSwitcher />
                    </nav>
                    <div className="dropdown" ref={dropdownRef}>
                        ...
                        <div className={`dropdown-menu dropdown-menu-right rounded-0 ${isDropdownOpen ? 'active' : ''}`}>
                            <div className="social-icons-header">
                                ...
                                <Link to="/map" className="dropdown-item desktop-unactive" onClick={handleDropdownItemClick}>{t('header.map')}</Link>
                            </div>
                            <div className="social-icons-header">
                                <FontAwesomeIcon className="social-mobile" icon={faBars} />
                                <Link to="/craigslist" className="dropdown-item desktop-unactive" onClick={handleDropdownItemClick}>{t('header.craigslist')}</Link>
                            </div>
                            {!isAuthentication && <>
                                <div className="social-icons-header">
                                    <FontAwesomeIcon icon={faArrowRightToBracket} />
                                    <Link to="/sign-up?view=login" className="dropdown-item" onClick={handleDropdownItemClick}>{t('header.login')}</Link>
                                </div>
                                <div className="social-icons-header">
                                    <FontAwesomeIcon icon={faAddressCard} />
                                    <Link to="/sign-up?view=register" className="dropdown-item" onClick={handleDropdownItemClick}>{t('header.register')}</Link>
                                </div>
                            </>}
                            {isAuthentication && <>
                                ...
                                <div className="social-icons-header"><FontAwesomeIcon icon={faArrowRightFromBracket} />
                                    <Link to="/logout" className="dropdown-item" onClick={handleDropdownItemClick}>{t('header.logout')}</Link>
                                </div>
