...
export const MentorCard = ({ mentor, index }) => {
  const { t } = useTranslation();
  
  // Четни индекси (0, 2, 4...) - снимка ляво
  // Нечетни индекси (1, 3, 5...) - снимка дясно
      ...
      <div className="mentor-section-image">
        <div className="mentor-image-background">
          <div className="mentor-image-frame">
            <img 
              src={mentor.avatar} 
              alt={`${mentor.name} - ${mentor.specialization} ментор`}
            />
          </div>
          
          {/* Badge */}
          <div className={`mentor-section-badge ${mentor.availability === 'available' ? 'available' : 'busy'}`}>
            {mentor.availability === 'available' && <span className="mentor-section-badge-dot"></span>}
            {t(`digiBridge.mentorsPage.card.${mentor.availability === 'available' ? 'available' : 'busy'}`)}
          </div>
        </div>
      </div>
      ...
      <div className="mentor-section-content">
        ...
        <div className="mentor-section-stats">
          ...
          <div className="mentor-section-stat">
            ...
            <div className="mentor-stat-info">
              <span className="mentor-stat-number">{mentor.rating.toFixed(1)}</span>
              <span className="mentor-stat-label">Рейтинг</span>
            </div>
          </div>
          ...
          <div className="mentor-section-stat">
            ...
            <div className="mentor-stat-info">
              ...
              <span className="mentor-stat-label">Опит</span>
            </div>
          </div>
        </div>

        {/* CTA Button */}
        <button className="mentor-section-button">
          <span>Виж профил</span>
