...
export const InitiativesList = () => {
  ...
  // Mock функция за взимане на данни
  ...
  useEffect(() => {
    ...
    if (showActiveOnly) {
      filtered = filtered.filter(initiative => initiative.status === 'active');
    }

    setFilteredInitiatives(filtered);
  }, [searchTerm, selectedCategory, selectedStatus, selectedLocation, showActiveOnly, initiatives]);
  ...
  return (
    <div className="initiatives-main-container">
      {/* Header */}
      <div className="initiatives-header-section">
        <h1 className="initiatives-main-title">Project Search</h1>
        
        {/* Search Bar with Checkbox */}
        <div className="initiatives-search-row">
          <div className="initiatives-search-wrapper">
            <svg className="initiatives-search-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <input
              ...
              className="initiatives-main-search"
            />
          </div>
          
          <label className="initiatives-checkbox-wrapper">
            <input
              type="checkbox"
              checked={showActiveOnly}
              onChange={(e) => setShowActiveOnly(e.target.checked)}
              className="initiatives-checkbox-input"
            />
            <span className="initiatives-checkbox-text">show only open calls</span>
          </label>
        </div>

        {/* Filter Dropdowns */}
        <div className="initiatives-filters-row">
          <div className="initiatives-dropdown-wrapper">
            <select 
              value={selectedCategory} 
              onChange={(e) => setSelectedCategory(e.target.value)}
              className="initiatives-filter-select"
            >
              <option value="">Areas of support</option>
              {categories.map(category => (
                <option key={category} value={category}>{category}</option>
              ))}
            </select>
            <svg className="initiatives-dropdown-arrow" width="12" height="8" viewBox="0 0 12 8">
          ...
          <div className="initiatives-dropdown-wrapper">
            <select 
              ...
              className="initiatives-filter-select"
            >
              <option value="">Search recommendations</option>
        ...
        {/* Results Header */}
        <div className="initiatives-results-header">
          <h2 className="initiatives-results-count">
            {filteredInitiatives.length} Projects in {locations.length} Countries
          </h2>
          
          <button 
            className={`initiatives-map-btn ${showMap ? 'active' : ''}`}
      ...
      {filteredInitiatives.length > 0 ? (
        ...
        <div className="initiatives-no-results">
          <p>Няма намерени инициативи със зададените критерии.</p>
        </div>
      )}
    </div>
  );
