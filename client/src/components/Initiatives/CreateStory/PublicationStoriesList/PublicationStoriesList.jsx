...
const PublicationStoriesList = () => {
  ...
  const loadStories = useCallback(async () => {
    try {
      ...
      await getAllStories(1, true, false);
    } catch (error) {
  ...
  const loadPage = useCallback(async (page) => {
    try {
      ...
      const response = await getAllPublications(page, true, false);
  ...
  const contentData = useMemo(() => {
    if (contentType === 'stories') {
      return contextStories || [];
    }
    return publications || [];
  }, [contentType, publications, contextStories]);
  ...
  }, [contentType, loadPage, loadStories]);
  ...
  useEffect(() => {
    if (contentData.length > 0) {
      ...
      if (contentType === 'publications') {
        ...
        const existingViewed = ViewedStoriesManager.getViewedStories();
        setViewedStories(existingViewed);
      }
  ...
  useEffect(() => {
    const checkForNewViewed = () => {
      if (contentType === 'publications') {
        ...
        const currentViewed = ViewedStoriesManager.getViewedStories();
        if (currentViewed.length !== viewedStories.length) {
          setViewedStories(currentViewed);
        }
      }
    };
  ...
  }, [viewedPublications.length, viewedStories.length, contentType]);
  ...
  const shouldShowRecentlyViewed = contentType === 'publications'
    ? viewedPublications.length > 0
    : viewedStories.length > 0;

  const filteredViewedContent = contentType === 'publications'
    ? viewedPublications.filter(viewed => viewed.id !== featuredPublication?.id)
    : viewedStories.filter(viewed => viewed.id !== featuredPublication?.id);
        ...
        <div className="ps-content-wrapper">
          {(isInitialLoad || isLoading) && contentData.length === 0 ? (
            ...
            <>
              ...
              {shouldShowRecentlyViewed && filteredViewedContent.length > 0 && (
                <section className="ps-recently-viewed-section">
                  ...
                  <div className="ps-recently-viewed-grid">
                    {filteredViewedContent.map((content, index) => (
                      <PublicationStoriesCard
