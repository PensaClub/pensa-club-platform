...
const ArticlesList = () => {
  ...
  const metaData = useMemo(() => {
    ...
    if (isSearchActive && searchTerm) {
      title = t('articles.meta.searchTitle', {
        term: searchTerm,
        defaultValue: `Търсене: ${searchTerm} | Статии | ${siteName}`
      });
      description = t('articles.meta.searchDescription', {
        term: searchTerm,
        count: filteredArticles.length,
        defaultValue: `Намерени ${filteredArticles.length} статии за "${searchTerm}". Открийте ресурси за дигитална грамотност.`
      });
      keywords = `${searchTerm}, търсене, статии, дигитална грамотност, пенсионери, Pensa Club`;
    } else if (currentPage > 1) {
      title = t('articles.meta.pageTitle', {
        page: currentPage,
        defaultValue: `Статии - Страница ${currentPage} | ${siteName}`
      });
      description = t('articles.meta.pageDescription', {
        page: currentPage,
        defaultValue: `Разгледайте нашите статии за дигитална грамотност за пенсионери - Страница ${currentPage}. Полезни съвети и ръководства.`
      });
      keywords = 'дигитална грамотност, пенсионери, статии, образование, технологии, Pensa Club';
    } else {
      title = t('articles.meta.title', {
        defaultValue: `Статии за дигитална грамотност | ${siteName}`
      });
      description = t('articles.meta.description', {
        defaultValue: 'Полезни статии за дигитална грамотност, технологии и активен живот за пенсионери. Съвети за интернет безопасност, електронна комуникация и много повече.'
      });
      keywords = 'дигитална грамотност, пенсионери, статии, технологии, образование, Pensa Club, възрастни, интернет сигурност, съвети';
    }

    return { title, description, keywords };
  }, [isSearchActive, searchTerm, filteredArticles.length, currentPage, t]);
  // Structured Data за статии
  const structuredData = useMemo(() => ({
    ...
    "inLanguage": [i18n.language, "bg", "de", "en"],
    "publisher": {
      "@type": "Organization",
      "name": "Pensa Club",
