...
const getMetaData = () => {
  ...
  const siteName = "Pensa Club";
  
  let title, description, keywords;
  
  if (isSearchActive && searchTerm) {
    // Мета данни при търсене
    title = t('articles.meta.searchTitle', { term: searchTerm, defaultValue: `Search: ${searchTerm} | Articles | ${siteName}` });
    description = t('articles.meta.searchDescription', { 
      term: searchTerm, 
      count: filteredArticles.length,
      defaultValue: `Found ${filteredArticles.length} articles for "${searchTerm}". Discover digital literacy resources and helpful guides for seniors.` 
    });
    keywords = t('articles.meta.searchKeywords', { 
      term: searchTerm,
      defaultValue: `${searchTerm}, search, articles, digital literacy, seniors, technology guides` 
    });
  } else if (currentPage > 1) {
    // Мета данни за пагинация
    title = t('articles.meta.pageTitle', { page: currentPage, defaultValue: `Articles - Page ${currentPage} | ${siteName}` });
    description = t('articles.meta.pageDescription', { 
      page: currentPage,
      defaultValue: `Browse our digital literacy articles and guides for seniors - Page ${currentPage}. Learn technology, stay connected, and discover new digital skills.` 
    });
    keywords = t('articles.meta.keywords', { defaultValue: 'digital literacy, seniors, technology guides, articles, education, pensa club' });
  } else {
    // Основни мета данни
    title = t('articles.meta.title', { defaultValue: `Articles | ${siteName}` });
    description = t('articles.meta.description', { 
      defaultValue: 'Discover helpful articles and guides about digital literacy, technology, and staying connected. Resources designed specifically for seniors and mature adults.' 
    });
    keywords = t('articles.meta.keywords', { defaultValue: 'digital literacy, seniors, technology guides, articles, education, pensa club, elderly, mature adults, technology learning' });
  }

  return { title, description, keywords, currentUrl, siteName };
};
...
const metaData = getMetaData();
  ...
  return (
    ...
    <Helmet>
      ...
      <meta property="og:locale" content={i18n.language === 'bg' ? 'bg_BG' : i18n.language === 'de' ? 'de_DE' : 'en_US'} />
