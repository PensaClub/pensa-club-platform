...
export const AllAnnouncements = () => {
  ...
  const handleLegendClickBar = (dataKey) => {
    setHiddenBar(hiddenBar.includes(dataKey) ? hiddenBar.filter((key) => key !== dataKey) : [...hiddenBar, dataKey]);
  };

  const handleLegendClickLine = (dataKey) => {
    setHiddenLine(hiddenLine.includes(dataKey) ? hiddenLine.filter((key) => key !== dataKey) : [...hiddenLine, dataKey]);
  };

  const handleLegendClickPie = (name) => {
    setHiddenPie(hiddenPie.includes(name) ? hiddenPie.filter((key) => key !== name) : [...hiddenPie, name]);
  };
